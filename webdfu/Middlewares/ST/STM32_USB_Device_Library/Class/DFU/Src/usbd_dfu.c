                              ...
                              uint8_t cfgidx)
{
    ...
    pdev->pClassData = USBD_malloc(sizeof(USBD_DFU_HandleTypeDef)); // 分配堆空间
    ...
    switch (hdfu->dev_state)
    {
        case   DFU_STATE_DNLOAD_SYNC:
            if (hdfu->wlength != 0U)
            {
                ...
                printf("hdfu->wblock_num =%d;hdfu->buffer.d8[0] = %d",hdfu->wblock_num,hdfu->buffer.d8[0]);
        ...
        case   DFU_STATE_MANIFEST_SYNC :
            ...
            {
                if ((hdfu->manif_state == DFU_MANIFEST_COMPLETE) && ((USBD_DFU_CfgDesc[(11U + (9U * USBD_DFU_MAX_ITF_NUM))]) & 0x04U))
                {
                    hdfu->dev_state = DFU_STATE_IDLE;
...
static void DFU_Leave(USBD_HandleTypeDef *pdev)
{
    ...
    {
        printf("DFU_STATE_MANIFEST_WAIT_RESET\r\n");
